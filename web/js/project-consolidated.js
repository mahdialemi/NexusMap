        ...
        function renderConsolidatedPorts() {
            ...
            for (const p of ports) {
                ...
                const histBtn = '<button class="btn btn-secondary btn-sm btn-history" onclick="event.stopPropagation();openPortHistory(\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\')" title="' + t('consolidated.view_scan_history') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>';
                const nseScripts = getPortNSEScripts(p.ip, p.port, p.protocol);
                const nseBtn = nseScripts.length > 0 ? '<button class="btn btn-secondary btn-sm btn-nse" onclick="event.stopPropagation();toggleNSERow(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\')" title="' + t('consolidated.view_nse') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg><span class="nse-count">' + nseScripts.length + '</span></button>' : '<span style="color:var(--text-muted);font-size:0.7rem;">-</span>';
                const checked = consolidatedSelected.has(key) ? 'checked' : '';
                html += '<tr data-ip="' + esc(p.ip) + '" data-port="' + p.port + '" data-proto="' + esc(p.protocol) + '">';
                html += '<td><input type="checkbox" class="cons-cb" data-key="' + esc(key) + '" ' + checked + ' onchange="toggleConsolidatedSelect(\'' + escAttr(key) + '\', this.checked)"></td>';
                html += '<td class="mono">' + esc(p.ip) + '</td>';
                html += '<td class="mono editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'mac\',\'' + escAttr(p.mac || '') + '\')">' + (esc(p.mac) || '-') + getHostEditRevertHtml(p.ip) + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'hostname\',\'' + escAttr(p.hostname || '') + '\')">' + (esc(p.hostname) || '-') + getHostEditRevertHtml(p.ip) + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'os\',\'' + escAttr(p.os || '') + '\')">' + (esc(p.os) || '-') + getHostEditRevertHtml(p.ip) + '</td>';
                html += '<td>' + stateBadge(p.host_status) + '</td>';
                html += '<td>' + p.port + '</td>';
                html += '<td>' + esc(p.protocol) + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'state\',\'' + escAttr(p.state) + '\')">' + stateBadge(p.state) + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'service\',\'' + escAttr(p.service || '') + '\')">' + (esc(p.service) || '-') + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'version\',\'' + escAttr(p.version || '') + '\')">' + (esc(p.version) || '-') + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'product\',\'' + escAttr(p.product || '') + '\')">' + (esc(p.product) || '-') + '</td>';
                html += '<td class="editable-cell" ondblclick="editConsolidatedCell(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\',\'extra_info\',\'' + escAttr(p.extra_info || '') + '\')" title="' + esc(p.extra_info || '') + '">' + (esc(p.extra_info) || '-') + '</td>';
                html += '<td style="text-align:center;">' + p.change_count + '</td>';
                html += '<td>' + formatDate(p.last_seen) + '</td>';
                html += '<td title="' + esc(p.note_preview || '') + '"><button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();toggleNoteRow(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\')" title="' + t('consolidated.edit_note') + '" style="padding:2px 6px;font-size:0.75rem;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></td>';
        ...
        function renderLabelDropdown(p) {
            ...
            for (const o of LABEL_OPTIONS) {
                ...
                html += `<div class="label-menu-item"${sel} onclick="event.stopPropagation();setPortLabel('${escAttr(p.ip)}',${p.port},'${escAttr(p.protocol)}','${o.value}');this.closest('.label-menu').style.display='none';" style="padding:5px 10px;cursor:pointer;font-size:0.75rem;display:flex;align-items:center;gap:6px;">`;
        ...
        function renderConsolidatedPortsGrouped(container, ports) {
            ...
            for (const ip of sortedIPs) {
                ...
                for (const p of hostPorts) {
                    ...
                    const histBtn = '<button class="btn btn-secondary btn-sm btn-history" onclick="event.stopPropagation();openPortHistory(\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\')" title="' + t('consolidated.view_scan_history') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></button>';
                    const nseScripts = getPortNSEScripts(p.ip, p.port, p.protocol);
                    const nseBtn = nseScripts.length > 0 ? '<button class="btn btn-secondary btn-sm btn-nse" onclick="event.stopPropagation();toggleNSERow(this,\'' + escAttr(p.ip) + '\',' + p.port + ',\'' + escAttr(p.protocol) + '\')" title="' + t('consolidated.view_nse') + '" style="display:inline-flex;align-items:center;gap:2px;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg><span class="nse-count">' + nseScripts.length + '</span></button>' : '<span style="color:var(--text-muted);font-size:0.7rem;">-</span>';
                    ...
                    html += '<td><input type="checkbox" class="cons-cb" data-key="' + esc(pkey) + '" ' + checked + ' onchange="toggleConsolidatedSelect(\'' + escAttr(pkey) + '\', this.checked)"></td>';
        ...
        function renderNotesPage() {
            ...
            for (const n of pageData) {
                ...
                html += '<button class="btn btn-secondary btn-sm" onclick="openPortNote(\'' + escAttr(n.ip) + '\',' + n.port + ',\'' + escAttr(n.protocol) + '\')" title="' + t('common.edit') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>';
                html += ' <button class="btn btn-secondary btn-sm" onclick="deleteNoteFromTab(\'' + escAttr(n.ip) + '\',' + n.port + ',\'' + escAttr(n.protocol) + '\')" title="' + t('app.delete') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>';
                html += '</td>';
                html += '</tr>';
            }
            html += '</tbody></table>';
        ...
        function renderScriptsPage() {
            ...
            for (const s of pageData) {
                ...
                html += '<td style="white-space:nowrap;"><button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();showScriptModalFromData(\'' + escAttr(s.script_id) + '\',\'' + escAttr(s.ip) + '\',' + s.port + ')" title="' + t('consolidated.view_output_short') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>';
                html += ' <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();goToConsolidatedPort(\'' + escAttr(s.ip) + '\',' + s.port + ')" title="' + t('consolidated.view_in_assets') + '"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></button></td>';
                html += '</tr>';
            ...
            try {
                ...
                if (edits && edits.length > 0) {
                    ...
                    for (const e of edits) {
                        ...
                        html += '</div></div>';
                        if (isApplied) {
                            html += '<button class="btn btn-secondary btn-sm" onclick="revertEdit(' + e.edit_id + ',\'' + escAttr(ip) + '\',' + port + ',\'' + escAttr(protocol) + '\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>' + t('consolidated.revert_btn') + '</button>';
                        } else {
                            html += '<button class="btn btn-secondary btn-sm" onclick="applyEdit(' + e.edit_id + ',\'' + escAttr(ip) + '\',' + port + ',\'' + escAttr(protocol) + '\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>' + t('consolidated.reapply_btn') + '</button>';
                        }
                        html += '</div>';
                    }
                }
